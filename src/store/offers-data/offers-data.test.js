...
describe(`Reducers work correctly`, () => {
  ...
  it(`Reducer should update offers`, () => {
    ...
    expect(offersData(
        state,
        updateOffers(mockOffers[0])
    )).toEqual({
      offers: state.offers,
      favoriteOffers: adaptOffersToClient(mockOffers)
    });
  });
  ...
  it(`Reducer should change favorite offers and load status`, () => {
    const state = {
      favoriteOffers: [],
      isFavoriteOffersLoaded: false
    };
    expect(offersData(state, loadFavoriteOffers(mockOffers)))
      .toEqual({
        favoriteOffers: adaptOffersToClient(mockOffers),
